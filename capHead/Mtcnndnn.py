...
class MTCNNDetector(object):
    def __init__(self, model_dir="model", factor = 0.709,
                 threshold = [0.6, 0.7, 0.7] ):
        self.threshold = threshold  #人脸框得分阈值
        self.factor = factor        #生成图像金字塔时的缩放系数
    ...
    def generateBoundingBox(self,map,reg,scale,threshold):
        ...
        a = np.where(map.flatten('C')>=threshold)

        if y.shape[0]==1:
            y=y.T
            x=x.T
            score=np.reshape(map.flatten('C')[a[0]],(1,1))
            dx1=dx1.T
            dy1=dy1.T
            dx2=dx2.T
            dy2=dy2.T
            ...
            score=map.flatten('C')[a[0]]
            score=np.reshape(score, (a[0].shape[0],1))
            
        dx1N=np.reshape(dx1.flatten('C')[a[0]], (a[0].shape[0],1))
        dy1N=np.reshape(dy1.flatten('C')[a[0]], (a[0].shape[0],1))
        dx2N=np.reshape(dx2.flatten('C')[a[0]], (a[0].shape[0],1))
        dy2N=np.reshape(dy2.flatten('C')[a[0]], (a[0].shape[0],1))  
